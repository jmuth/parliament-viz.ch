...
This project represents the final point of {\bf Applied Data Analysis} course, taught by Michele Catasta in 2016. We put into practice the analysis of real-world data, following the whole pipeline, from {\bf scraping} to {\bf interactive visualization}. We experiment what are the \textbf{real-world challenges} encountered in such a project. Our case study focuses on the data available on \url{hwww.parliament.ch} which describe all publicly available activities of Swiss parliamentary deputies.
...
Since the \textbf{Freedom of Information Act} entered into force (on 1 July 2006) a huge amount of data related to political activities are freely available to the public. However, it represents a large number of documents hardly understandable. We suggest an interface giving \textbf{light overview} of Swiss parliamentarians.
...
We develop a python scrapper that bypass these restrictions.
...
\item 3 different IDs to designate a parliamentary
\end{itemize}
...
As working on data involving public peoples, we forbid ourselves for any \textbf{subjective interpretations}. Instead, we focus on a visualization tool from which other people (e.g. journalist) can draw conclusions. Hence, it needed to be transparent enough to be trusted.
