...
\node [shift={(-16.4 cm,-6cm)}] at (current page.north east) {\includegraphics[height=7cm]{img/ada_logo} ~ ~ ~ ~ ~ ~ \includegraphics[height=7cm]{img/epfl_logo}}; 
\end{tikzpicture} 
\begin{tikzpicture}[remember picture,overlay] 
\node [shift={(10cm,-6cm)}] at (current page.north west) {\includegraphics[height=7cm]{img/parlament_logo}}; 
\end{tikzpicture} 
}
...
<<<<<<< Updated upstream
This project represents the final point of {\bf Applied Data Analysis} course, taught by Michele Catasta in 2016. We put into practice the analysis of real-world data, following the whole pipeline from {\bf scraping} to {\bf interactive visualization}. By doing so we experienced the \textbf{real-world challenges} that arise in such a project. Our case study focuses on the data available on \url{hwww.parliament.ch} which describe all publicly available activities of the Swiss Parliament.
=======
This project represents the final point of the {\bf Applied Data Analysis} course taught by Michele Catasta in 2016. We put into practice an analysis of real-world data and followed the whole pipeline, from {\bf scraping} to an {\bf interactive visualization}. By doing so we experienced first-hand the \textbf{real-world challenges} that arise in such a project. Our case study focus on the data available on \url{hwww.parliament.ch} which describes all publicly available activities of the Swiss Parliament.
>>>>>>> Stashed changes
...
=======
Since the \textbf{The Freedom of Information Act} entered into force (on 1 July 2006) a huge amount of data related to political activities is freely available to the public. However, it represents a big amount of documents hardly understandable. We suggest an interface giving a \textbf{light overview} of the Swiss parliamentary activities.
>>>>>>> Stashed changes
...
=======
The Parliament IT service provided us with an API, which allows direct access to database tables. However, it presented some restrictions:

\begin{itemize}
\item \textbf{Limited} number ofreturned items per request
\item A \textbf{timeout} so short it prevented us from using \texttt{skip} to access the last elements of big tables.
\end{itemize}
...
=======
As we are working on data involving public people, we refrain from giving any \textbf{subjective interprepretation}. Instead, we focus on a visualization tool that other people (e.g. journalists) can use to draw conclusions. Hence, it needs to be transparent enough to be trusted.
>>>>>>> Stashed changes
...
\small{\rmfamily{A warm thank you to \textbf{Alain Rebetez}, political journalist, and \textbf{Philippe Nantermod}, PLR deputy in the National Council, for their expertise in the Swiss parliamentary system and their external point of view on the project.}} \\
